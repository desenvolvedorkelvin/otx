...
void Items::parseItemNode(xmlNodePtr itemNode, uint32_t id)
{
	...
	for(xmlNodePtr itemAttributesNode = itemNode->children; itemAttributesNode; itemAttributesNode = itemAttributesNode->next)
	{
		...
		else if(tmpStrValue == "premiumdays")
		{
			...
			if (readXMLInteger(itemAttributesNode, "value", intValue)) {
				it.getAbilities()->specialSkills[SPECIALSKILL_LIFELEECH] = intValue;
			}
		}
		else if (tmpStrValue == "manaleech") {
			if (readXMLInteger(itemAttributesNode, "value", intValue)) {
				it.getAbilities()->specialSkills[SPECIALSKILL_MANALEECH] = intValue;
			}
		}
		else if (tmpStrValue == "dodgechance") {
			if (readXMLInteger(itemAttributesNode, "value", intValue)) {
				it.getAbilities()->specialSkills[SPECIALSKILL_DODGECHANCE] = intValue;
			}
		}
		else if (tmpStrValue == "criticalchance") {
			if (readXMLInteger(itemAttributesNode, "value", intValue)) {
				it.getAbilities()->specialSkills[SPECIALSKILL_CRITICALCHANCE] = intValue;
			}
		}
		else if (tmpStrValue == "criticaldamage") {
			if (readXMLInteger(itemAttributesNode, "value", intValue)) {
				it.getAbilities()->specialSkills[SPECIALSKILL_CRITICALDAMAGE] = intValue;
			}
		}
		else
