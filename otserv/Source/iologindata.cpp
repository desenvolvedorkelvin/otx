...
bool IOLoginData::savePlayer(Player* player, bool preSave/* = true*/, bool shallow/* = false*/)
{
	...
	for (const auto& it : player->getOpenContainers()) {
		if (it.second) {
			openContainers[it.second] = it.first;
		}
	}
	...
	if(!saveItems(player, itemList, stmt, openContainers))
		return false;
	...
	std::map<Container*, int>& openContainers)
{
	...
	for(ItemBlockList::const_iterator it = itemList.begin(); it != itemList.end(); ++it, ++runningId)
	{
		...
		if (Container* subContainer = item->getContainer()) {
			auto oc_it = openContainers.find(subContainer);
			if (oc_it == openContainers.end()) {
				subContainer->resetAutoOpen();
			} else {
				subContainer->setAutoOpen(oc_it->second);
			}
			stackList.emplace_back(subContainer, runningId);
		}
