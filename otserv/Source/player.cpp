	...
	if(autoCreateDepot)
	{
		...
		if(Container* container = locker->getContainer())
		{
			if(Depot* depot = container->getDepot())
			{
				...
				container->__internalAddThing(Item::CreateItem(ITEM_MARKET));
	...
	bool checkDefense/* = false*/, bool checkArmor/* = false*/, bool reflect/* = true*/, bool field/* = false*/, bool element/* = false*/)
{
	...
	if (attacker && combatType != COMBAT_HEALING) {
		int32_t dodgeChance = getSpecialSkill(SPECIALSKILL_DODGECHANCE);
		if (dodgeChance > 0 && random_range(1, 100) <= dodgeChance) {
			g_game.addAnimatedText(getPosition(), COLOR_LIGHTGREEN, "Dodge!");
			damage = 0;
	...
	if (attackerPlayer) {
		int32_t criticalChance = attackerPlayer->getSpecialSkill(SPECIALSKILL_CRITICALCHANCE);
		int32_t criticalDamagePercent = attackerPlayer->getSpecialSkill(SPECIALSKILL_CRITICALDAMAGE);
		if (criticalChance > 0 && criticalDamagePercent > 0 && random_range(1, 100) <= criticalChance) {
			g_game.addAnimatedText(getPosition(), COLOR_DARKRED, "Critical!");
			damage += std::ceil(damage * (criticalDamagePercent / 100.));
		}
	}
	...
	if(damage <= 0)
	{
		...
		if (attackerPlayer) {
			int32_t lifeLeechPercent = attackerPlayer->getSpecialSkill(SPECIALSKILL_LIFELEECH);
			if (lifeLeechPercent != 0) {
				g_game.combatChangeHealth(COMBAT_HEALING, NULL, attacker, std::ceil(damage * (lifeLeechPercent / 100.)));
			}

			int32_t manaLeechPercent = attackerPlayer->getSpecialSkill(SPECIALSKILL_MANALEECH);
			if (manaLeechPercent != 0) {
				g_game.combatChangeMana(NULL, attacker, std::ceil(damage * (manaLeechPercent / 100.)));
			}
		}
	}
...
GuildEmblems_t Player::getGuildEmblem(const Creature* creature) const
{
	...
	return GUILDEMBLEM_NONE; //this check if player in guild or not and remove emblem from him. the color red, brown, green, where is change?

	if(player->isEnemy(this))
		return GUILDEMBLEM_ENEMY;

	if(player->getGuildId() == guildId && player->getGuildLevel() != GUILDLEVEL_LEADER)
		return GUILDEMBLEM_ALLY; //Here for guild members << ally = green

	if(player->getGuildLevel() == GUILDLEVEL_LEADER && player->getGuildId() == guildId)
		return GUILDEMBLEM_ENEMY; //here for leader enem = red

	return GUILDEMBLEM_NEUTRAL; // here return for all players not in guilds blue. or any other guild blue
...
bool Player::onKilledCreature(Creature* target, DeathEntry& entry)
{
	...
	if(!targetPlayer->hasAttacked(this) && target->getSkull() == SKULL_NONE && targetPlayer != this && (!guildId || !targetPlayer->getGuildId()) && (addUnjustifiedKill(targetPlayer, !enemy.war) || entry.isLast()))
...
bool Player::canWearOutfit(uint32_t outfitId, uint32_t addons)
{
	...
	if(it == outfits.end() || ((it->second.addons & addons) != addons && !hasCustomFlag(PlayerCustomFlag_CanWearAllAddons)))
		return false;
...
void Player::receiveMarketItem(uint16_t index)
{
	...
	if (Item* item = container->getItem(index)) {
		if (item->getWorth() != 0) {
			sendTextMessage((MessageClasses)22, "Voce nao pode anunciar moedas.");
			return;
		}
...
bool Player::removeMarketItem(uint16_t integgerAttribute, uint16_t amount, uint64_t price, uint16_t gender, uint16_t level, std::string ispokemon, std::string name, bool onlyoffers)
{
	...
	if (g_game.getMoney(this) < newPrice) {
		sendTextMessage((MessageClasses)22, "Voce nao possui dinheiro suficiente para anunciar este produto.");
		return false;
	}
	...
	if (count_row2 >= 10) {
		sendTextMessage((MessageClasses)22, "Voce nao pode anunciar mais produtos, limite: 10.");
		return false;
	}
...
bool Player::buyMarketItemOffer(uint64_t price, uint64_t transaction_id, uint64_t countSelectedScrollBar)
{
	if (g_game.getMoney(this) < price) {
		sendTextMessage((MessageClasses)22, "Voce nao possui dinheiro suficiente para comprar este produto.");
		return false;
	}
	...
	if ((result = db->storeQuery(query.str())))
	{
		...
		/* if (result->getDataInt("player_id") == getGUID()) {
			sendTextMessage("Voce nao pode comprar um produto que voce mesmo anunciou.");
			return false;
		} */
		...
		if ((int)countSelectedScrollBar > result->getDataInt("amount")) {
			sendTextMessage((MessageClasses)22, "Voce nao pode comprar mais do que tem de oferta.");
			return false;
		}
		...
		sendTextMessage((MessageClasses)22, "A oferta que esteja tentando comprar nao esteja mais disponivel.");
		return false;
	}
	return true;
}
...
bool Player::makeMarketOfferRemoveItem(uint16_t index, uint64_t transaction_id)
{
	...
	if (getStorage(storage_str.str(), storage) && storage == "offerted") {
		sendTextMessage((MessageClasses)22, "Voce ja realizou uma oferta para este item.");
		return false;
	}
...
bool Player::confirmMarketOfferToPlayer(uint64_t transaction_id, std::string item_seller, uint16_t index) // Index is not in using!!
{
	...
	{
		sendTextMessage((MessageClasses)22, "Voce deve colocar pelo menos um item para poder ofertar.");
		return false;
	}
	...
	{
		/* if (result->getDataInt("player_id") == getGUID()) {
			sendTextMessage("Voce nao pode ofertar para um produto que voce mesmo anunciou.");
			return false;
		} */
		...
		sendTextMessage((MessageClasses)22, "Este item nao esta mais disponivel para oferta, atualize a pagina.");
		return true;
	}
	...
	sendTextMessage((MessageClasses)22, "Sua oferta foi realizada com sucesso, o vendedor sera notificado sobre sua oferta.");
...
{
	if (!g_config.getBool(ConfigManager::AUTO_OPEN_CONTAINERS)) {
		return;
	}
