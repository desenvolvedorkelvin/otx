...
void Party::broadcastPartyLoot(const std::string& loot)
{
	...
	leader->sendChannelMessage("", loot, MSG_CHANNEL_HIGHLIGHT, CHANNEL_LOOT);
	...
	if (!memberList.empty())
	{
		for (it = memberList.begin(); it != memberList.end(); ++it)
		{
			...
			(*it)->sendChannelMessage("", loot, MSG_CHANNEL_HIGHLIGHT, CHANNEL_LOOT);
		}
	}
}
...
void Party::shareExperience(double experience, Creature* target, bool multiplied)
{
	...
	double extraExperience = (((double)g_config.getNumber(ConfigManager::EXTRA_PARTY_PERCENT_MEMBER) * shareExperience) / 100);
	double tmpExperience = shareExperience + extraExperience;

	int16_t countMembers = 1; // starts with 1 to count leader
	int32_t extraLimitMembers = g_config.getNumber(ConfigManager::EXTRA_PARTY_MEMBER_LIMIT);

	leader->onGainSharedExperience(tmpExperience, target, multiplied);
	for(PlayerVector::iterator it = memberList.begin(); it != memberList.end(); ++it)
	{
		tmpExperience = shareExperience;
		if (countMembers < extraLimitMembers)
			tmpExperience += extraExperience;
