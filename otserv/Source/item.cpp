...
Attr_ReadValue Item::readAttr(AttrTypes_t attr, PropStream& propStream)
{
	switch(attr)
	{
		...
		{
			int8_t autoOpen;
			if (!propStream.getType(autoOpen)) {
				return ATTR_READ_ERROR;
			}

			setAttribute("autoopen", autoOpen);
			break;
		}
...
bool Item::serializeAttr(PropWriteStream& propWriteStream) const
{
	...
	int8_t autoOpen = getAutoOpen();
	if (autoOpen >= 0) {
		propWriteStream.addByte(ATTR_AUTOOPEN);
		propWriteStream.addType(autoOpen);
	}
	...
	int32_t subType/* = -1*/, bool addArticle/* = true*/)
{
	...
	else if(it.weaponType != WEAPON_NONE)
	{
		...
		bool hasAbilities = it.hasAbilities();
		int32_t specialSkill = (item ? item->getSpecialSkill(SPECIALSKILL_LIFELEECH) : (hasAbilities ? it.abilities->specialSkills[SPECIALSKILL_LIFELEECH] : 0));
		if (specialSkill != 0) {
			if (begin) {
				begin = false;
				s << " (";
			...
			s << "life leech: " << specialSkill << '%';
		}

		specialSkill = (item ? item->getSpecialSkill(SPECIALSKILL_MANALEECH) : (hasAbilities ? it.abilities->specialSkills[SPECIALSKILL_MANALEECH] : 0));
		if (specialSkill != 0) {
			if (begin) {
				begin = false;
				s << " (";
			...
			s << "mana leech: " << specialSkill << '%';
		}

		specialSkill = (item ? item->getSpecialSkill(SPECIALSKILL_DODGECHANCE) : (hasAbilities ? it.abilities->specialSkills[SPECIALSKILL_DODGECHANCE] : 0));
		if (specialSkill != 0) {
			if (begin) {
				begin = false;
				s << " (";
			...
			s << "dodge chance: " << specialSkill << '%';
		}

		int32_t criticalChance = (item ? item->getSpecialSkill(SPECIALSKILL_CRITICALCHANCE) : (hasAbilities ? it.abilities->specialSkills[SPECIALSKILL_CRITICALCHANCE] : 0));
		int32_t criticalDamage = (item ? item->getSpecialSkill(SPECIALSKILL_CRITICALDAMAGE) : (hasAbilities ? it.abilities->specialSkills[SPECIALSKILL_CRITICALDAMAGE] : 0));
		if (criticalChance > 0 && criticalDamage > 0) {
			if (begin) {
				begin = false;
				s << " (";
			} else {
			...
			s << "critical chance: " << criticalChance << "%, critical damage: " << criticalDamage << '%';
		}
...
int8_t Item::getAutoOpen() const
{
	bool ok;
	int8_t v = getIntegerAttribute("autoopen", ok);
	if (ok) {
		return v;
	}
	return -1;
...
void Item::setAutoOpen(int8_t value)
{
	setAttribute("autoopen", value);
}

void Item::resetAutoOpen()
{
	eraseAttribute("autoopen");
}

int32_t Item::getSpecialSkill(SpecialSkills_t skill) const
{
	std::string attribute;
	switch (skill) {
		case SPECIALSKILL_LIFELEECH: {
			attribute = "lifeleech";
			break;
		}

		case SPECIALSKILL_MANALEECH: {
			attribute = "manaleech";
			break;
		}
		...
		case SPECIALSKILL_CRITICALCHANCE: {
			attribute = "criticalchance";
			break;
		}

		case SPECIALSKILL_CRITICALDAMAGE: {
	...
	return items[id].abilities ? items[id].abilities->specialSkills[skill] : 0;
}
