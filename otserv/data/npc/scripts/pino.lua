local keywordHandler = KeywordHandler:new()
 ...
 local function creatureSayCallback(cid, type, msg)
	...
	if (msgcontains(msg, 'bring') and msgcontains(msg, 'me') and msgcontains(msg, 'to') and msgcontains(msg, 'svar') and (not npcHandler:isFocused(cid))) then
	...
	elseif (msgcontains(msg, 'bring') and msgcontains(msg, 'me') and msgcontains(msg, 'to') and msgcontains(msg, 'svargrond') and (not npcHandler:isFocused(cid))) then
		if doPlayerRemoveMoney(cid, 60) == TRUE then
		doTeleportThing(cid,{x = 32871, y = 31024, z = 4})
		npcHandler:addFocus(cid)
		else 
	...
	return true
		...
		local travelNode = keywordHandler:addKeyword({'svargrond'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'Do you want to fly to svargrond for 60 gold coins?'})
        ...
        keywordHandler:addKeyword({'fly'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'I can take you to Svargrond, Femor hills and Darashia.'})
        keywordHandler:addKeyword({'travel'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'I only can offer you my services to Svargrond, Femor Hills and darashia.'})
